...
#include "vanillacashaddressvalidator.h"
...
{
    widget->setMaxLength(VanillacashAddressValidator::MaxAddressLength);
    widget->setValidator(new VanillacashAddressValidator(parent));
    widget->setFont(vanillacashAddressFont());
}
...
bool parseVanillacashURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // NovaCoin: check prefix
    if(uri.scheme() != QString("vanillacash"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!VanillacashUnits::parse(VanillacashUnits::XVC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseVanillacashURI(QString uri, SendCoinsRecipient *out)
{
    // Convert vanillacash:// to vanillacash:
    //
    //    Cannot handle this later, because vanillacash:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("vanillacash://"))
    {
        uri.replace(0, 12, "vanillacash:");
    }
    QUrl uriInstance(uri);
    return parseVanillacashURI(uriInstance, out);
}
...
void openConfigfile()
{
    ...
    /* Open vanillacash.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Vanillacash.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Vanillacash.lnk
...
{
    return GetAutostartDir() / "vanillacash.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a vanillacash.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Vanillacash\n";
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Vanillacash-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  vanillacash-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Vanillacash-Qt"));
