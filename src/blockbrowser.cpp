...
std::string getOutputs(std::string txid)
{
    ...
    for (unsigned int i = (tx.IsCoinStake() ? 1 : 0); i < tx.vout.size(); i++)
    {
		...
		str.append(CVanillacashAddress(address).ToString());
        str.append(": ");
        str.append(amount);
        str.append(" XVC");
        str.append("\n");
...
void BlockBrowser::updateExplorer(bool block)
{
    if(block)
	{
        ...
        if (pindex->IsProofOfStake()) {
            ...
            ui->moneySupplyBox->setText(QString::number(getMoneySupply(height), 'f', 6) + " XVC");
    }

    else {
        std::string txid = ui->txBox->text().toUtf8().constData();
        ui->valueBox->setText(QString::number(getTxTotalValue(txid), 'f', 6) + " XVC");
        ...
        ui->feesBox->setText(QString::number(getTxFees(txid), 'f', 6) + " XVC");
    }
}
