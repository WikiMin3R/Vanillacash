...
#else
    const char* pszModule = "vanillacash";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Vanillacash
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Vanillacash
    // Mac: ~/Library/Application Support/Vanillacash
    // Unix: ~/.vanillacash
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Vanillacash";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "vanillacash.conf"));
...
std::string rpcpass = "rpcpassword=" + str + "\n";
    ...
    {
        // Don't overwrite existing settings so command line settings override vanillacash.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "vanillacashd.pid"));
