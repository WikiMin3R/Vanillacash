...
{
    QString ret = QString("vanillacash:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non XVC unit input in lnReqAmount, we generate the URI with XVC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(VanillacashUnits::format(VanillacashUnits::XVC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
